...
public interface RetryPolicy {
    ...
    @InterfaceStability.Evolving class RetryAction {
        ...
        static final RetryAction FAILOVER_AND_RETRY =
     ...
     * @param failovers The number of times the method has failed over to a different backend implementation
     * @param isIdempotentOrAtMostOnce <code>true</code> if the method is {@link Idempotent} or {@link AtMostOnce} and
     * so can reasonably be retried on failover when we don't know if the previous attempt reached the server or not
     * @return <code>true</code> if the method should be retried, <code>false</code> if the method should not be retried
     * but shouldn't fail with an exception (only for void methods)
     * @throws Exception The re-thrown exception <code>e</code> indicating that the method failed and should not be
     * retried further
     */
    // TODO: 17/3/14 by zmyer
    RetryAction shouldRetry(Exception e, int retries, int failovers,
