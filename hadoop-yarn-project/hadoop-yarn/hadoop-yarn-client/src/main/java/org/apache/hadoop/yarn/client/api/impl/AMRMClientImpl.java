...
public class AMRMClientImpl<T extends ContainerRequest> extends AMRMClient<T> {
    ...
    //挂起的待释放的容器集合
    private Set<ContainerId> pendingRelease = new TreeSet<ContainerId>();
    ...
    //容器资源变更映射表
    protected final Map<ContainerId, SimpleEntry<Container, Resource>> change = new HashMap<>();
    ...
    // completed.
    //容器资源变更历史映射表
    final Map<ContainerId, SimpleEntry<Container, Resource>> pendingChange = new HashMap<>();
    ...
    // TODO: 17/3/26 by zmyer
    @VisibleForTesting AMRMClientImpl(ApplicationMasterProtocol protocol) {
        super(AMRMClientImpl.class.getName());
            ...
            try {
                ...
                synchronized (this) {
                    ...
                    for (RemoteRequestsTable remoteRequestsTable :
                        ...
                        for (ResourceRequestInfo<T> aRemoteRequestsTable : (Iterable<ResourceRequestInfo<T>>) remoteRequestsTable) {
                            addResourceRequestToAsk(aRemoteRequestsTable.remoteRequest);
                        }
                    }
            ...
            if (allocateResponse == null) {
                ...
                synchronized (this) {
                    ...
                    for (Map.Entry<ContainerId, SimpleEntry<Container, Resource>> entry :
                        ...
                        change.putIfAbsent(oldContainerId, new SimpleEntry<>(oldContainer, oldResource));
                    }
    ...
    public void unregisterApplicationMaster(FinalApplicationStatus appStatus,
        String appMessage, String appTrackingUrl) throws YarnException, IOException {
        ...
        if (req.getNodes() != null) {
            ...
            for (String node : dedupedNodes) {
                ...
                addResourceRequest(req.getPriority(), node,
                    req.getExecutionTypeRequest(), req.getCapability(), req, true, req.getNodeLabelExpression());
            }
        }
    ...
    RemoteRequestsTable<T> getTable(long allocationRequestId) {
        return remoteRequests.get(allocationRequestId);
    }

    // TODO: 17/3/26 by zmyer
    RemoteRequestsTable<T> putTable(long allocationRequestId, RemoteRequestsTable<T> table) {
        return remoteRequests.put(allocationRequestId, table);
    }
}
