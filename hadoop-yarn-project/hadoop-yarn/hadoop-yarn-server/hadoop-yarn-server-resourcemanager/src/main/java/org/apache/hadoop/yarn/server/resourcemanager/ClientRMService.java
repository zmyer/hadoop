...
public class ClientRMService extends AbstractService implements ApplicationClientProtocol {
    ...
    protected void serviceStart() throws Exception {
        ...
        this.server = rpc.getServer(ApplicationClientProtocol.class, this,
            clientBindAddress, conf, this.rmDTSecretManager,
        ...
        if (allowAccess) {
            ...
            if (appAttempt == null) {
                throw new ApplicationAttemptNotFoundException(
                    "ApplicationAttempt with id '" + appAttemptId + "' doesn't exist in RM.");
            }
    ...
    public GetApplicationAttemptsResponse getApplicationAttempts(
        ...
        if (allowAccess) {
            ...
            for (Map.Entry<ApplicationAttemptId, RMAppAttempt> entry : attempts.entrySet()) {
                listAttempts.add(entry.getValue()
                    .createApplicationAttemptReport());
            }
    ...
    public SubmitApplicationResponse submitApplication(SubmitApplicationRequest request)
        ...
        try {
            ...
            RMAuditLogger.logFailure(user, AuditConstants.SUBMIT_APP_REQUEST,
                ie.getMessage(), "ClientRMService", "Exception in submitting application",
        ...
        if (submissionContext.getApplicationType() == null) {
            ...
            if (submissionContext.getApplicationType().length() > YarnConfiguration.APPLICATION_TYPE_LENGTH) {
                //如果应用类型长度超过了限制,则直接截断
                submissionContext.setApplicationType(submissionContext.getApplicationType().substring(0,
        ...
        try {
            ...
            RMAuditLogger.logFailure(user, AuditConstants.SUBMIT_APP_REQUEST, e.getMessage(), "ClientRMService",
    ...
    private NodeReport createNodeReports(RMNode rmNode) {
        ...
        return BuilderUtils.newNodeReport(rmNode.getNodeID(), rmNode.getState(),
