...
public class ResourceTrackerService extends AbstractService implements ResourceTracker {
    ...
    public void loadDynamicResourceConfiguration(Configuration conf)
        ...
        try {
            ...
            InputStream drInputStream = this.rmContext.getConfigurationProvider()
                .getConfigurationInputStream(conf, YarnConfiguration.DR_CONFIGURATION_FILE);
    ...
    protected void serviceStart() throws Exception {
        ...
        if (conf.getBoolean(CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHORIZATION, false)) {
            InputStream inputStream =
                this.rmContext.getConfigurationProvider().getConfigurationInputStream(conf,
    ...
    public RegisterNodeManagerResponse registerNodeManager(
        ...
        if (oldNode == null) {
            ...
            this.rmContext
                .getDispatcher()
                .getEventHandler()
                .handle(new RMNodeReconnectEvent(nodeId, rmNode, request
    ...
    public NodeHeartbeatResponse nodeHeartbeat(NodeHeartbeatRequest request)
        ...
        if (!this.nodesListManager.isValidNode(nodeId.getHost())
            && !isNodeInDecommissioning(nodeId)) {
            String message = "Disallowed NodeManager nodeId: " + nodeId + " hostname: "
                + nodeId.getHost();
            LOG.info(message);
