...
public class FileSystemRMStateStore extends RMStateStore {
    ...
    @Override
    public synchronized void storeApplicationStateInternal(ApplicationId appId, ApplicationStateData appStateDataPB)
        throws Exception {
    ...
    // TODO: 17/4/4 by zmyer
    private void writeFileWithRetries(final Path outputPath, final byte[] data, final boolean makeUnreadableByAdmin)
        throws Exception {
        new FSAction<Void>() {
            @Override
            public Void run() throws Exception {
