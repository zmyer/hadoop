...
public class RMAppAttemptImpl implements RMAppAttempt, Recoverable {
    ...
    //超时状态转移对象
    private static final ExpiredTransition EXPIRED_TRANSITION = new ExpiredTransition();
    //应用执行失败状态转移对象
    private static final AttemptFailedTransition FAILED_TRANSITION = new AttemptFailedTransition();
    ...
    private static final StateMachineFactory<RMAppAttemptImpl,
        ...
        .addTransition(RMAppAttemptState.NEW, RMAppAttemptState.FINAL_SAVING,
            ...
            new FinalSavingTransition(new UnexpectedAMRegisteredTransition(),
                RMAppAttemptState.FAILED))
        ...
        .addTransition(RMAppAttemptState.SCHEDULED, RMAppAttemptState.FINAL_SAVING,
            ...
            new FinalSavingTransition(new BaseFinalTransition(
                RMAppAttemptState.KILLED), RMAppAttemptState.KILLED))
        .addTransition(RMAppAttemptState.SCHEDULED, RMAppAttemptState.FINAL_SAVING,
            RMAppAttemptEventType.FAIL, new FinalSavingTransition(FAILED_TRANSITION, RMAppAttemptState.FAILED))
        .addTransition(RMAppAttemptState.SCHEDULED,
    ...
    private void setTrackingUrlToRMAppPage(RMAppAttemptState stateToBeStored) {
        originalTrackingUrl = pjoin(WebAppUtils.getResolvedRMWebAppURLWithScheme(conf),
    ...
    private void setTrackingUrlToAHSPage(RMAppAttemptState stateToBeStored) {
        originalTrackingUrl = pjoin(WebAppUtils.getHttpSchemePrefix(conf) +
    ...
    public void handle(RMAppAttemptEvent event) {
        ...
        try {
            ...
            if ((oldState != getAppAttemptState()) && ((recoveredFinalState == null) ||
    ...
    public void recover(RMState state) {
        ApplicationStateData appState = state.getApplicationState().get(getAppAttemptId()
            .getApplicationId());
    ...
    private static class BaseTransition implements SingleArcTransition<RMAppAttemptImpl,
        RMAppAttemptEvent> {
