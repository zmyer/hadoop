...
public abstract class AbstractYarnScheduler
    ...
    public void completedContainer(RMContainer rmContainer,
        ...
        if (rmContainer.getExecutionType() == ExecutionType.GUARANTEED) {
            //如果当前的容器执行类型为保障性,则需要立即进入到完成流程处理
            ...
            //根据容器所在的节点中删除已经启动的容器对象
            getSchedulerNode(rmContainer.getNodeId()).releaseContainer(rmContainer.getContainer());
        }
    ...
    // TODO: 17/4/6 by zmyer
    protected void decreaseContainers(List<UpdateContainerRequest> decreaseRequests,
        SchedulerApplicationAttempt attempt) {
    ...
    public void normalizeRequest(AbstractResourceRequest ask) {
        SchedulerUtils.normalizeRequest(ask, getResourceCalculator(),
